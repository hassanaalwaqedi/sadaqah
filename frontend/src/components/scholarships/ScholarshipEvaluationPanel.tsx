...
import { CheckCircle2, AlertCircle, TrendingUp, Users, GraduationCap, DollarSign, HeartHandshake, UserCircle } from "lucide-react";
...
const defaultCriteria: Criterion[] = [
  { id: 'academic', name: 'الأداء الأكاديمي', icon: GraduationCap, maxScore: 30, description: 'المعدل التراكمي والسجل الأكاديمي السابق' },
  { id: 'financial_need', name: 'الحاجة المالية', icon: DollarSign, maxScore: 30, description: 'مستوى الدخل وعدد أفراد الأسرة' },
  { id: 'social_circumstances', name: 'الظروف الاجتماعية', icon: HeartHandshake, maxScore: 20, description: 'الحالة الاجتماعية والظروف الخاصة' },
  { id: 'leadership_volunteer', name: 'القيادة والتطوع', icon: Users, maxScore: 10, description: 'المشاركة المجتمعية والأنشطة اللامنهجية' },
  { id: 'interview', name: 'المقابلة الشخصية', icon: UserCircle, maxScore: 10, description: 'تقييم المقابلة والدوافع الشخصية' },
];

type ScholarshipEvaluationPanelProps = {
  assignmentId?: string;
  projectId: string;
  existingScores?: any[];
...
export function ScholarshipEvaluationPanel({ assignmentId, projectId, existingScores = [], onScoreSubmitted, readOnly = false }: ScholarshipEvaluationPanelProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await apiClient.post(`/scholarships/applications/${projectId}/score`, payload);
      setSuccess(true);
