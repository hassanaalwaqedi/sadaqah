...
export default function StudentApplicationTracker() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const data = res.data;
      data.messages = data.messages.filter(m => !m.is_internal);
      setDetails(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  async function handleSendMessage(e: React.FormEvent) {
    ...
    setIsSending(true);
    try {
      await apiClient.post(`/innovation/applications/${appId}/messages`, {
        message: newMessage,
        is_internal: false
      });
      setNewMessage('');
  ...
  const steps = ['submitted', 'under_review', 'under_judging', 'accepted'];
  const currentStepIndex = steps.indexOf(details.submission.status);
  const isRejected = details.submission.status === 'rejected';

  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center gap-2 text-sm text-surface-500 mb-4">
        <Link href="/portal/innovation/my-applications" className="hover:text-primary-600">
          طلباتي
        </Link>
        <ChevronRight className="w-4 h-4" />
        <span className="text-surface-900 dark:text-surface-100">{details.submission.title}</span>
      </div>
      ...
      <div className="glass-card p-6">
        ...
        <p className="text-surface-600 dark:text-surface-400">
          {details.event.name_ar} • مقدم في {formatDateTime(details.submission.created_at)}
        </p>
        ...
        <div className="mt-8 mb-4">
          <div className="flex items-center justify-between relative">
            ...
            {['تقديم الطلب', 'قيد المراجعة', 'التحكيم', 'القرار النهائي'].map((stepName, idx) => {
              ...
              return (
                <div key={idx} className="relative z-10 flex flex-col items-center gap-2 bg-surface-50 dark:bg-surface-900 px-2">
                  ...
                  <span className={`text-sm font-medium ${isCurrent || isCompleted ? 'text-surface-900 dark:text-white' : 'text-surface-500'}`}>
                    {stepName}
                  </span>
                </div>
              );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="glass-card p-6 flex flex-col h-[500px]">
            <h2 className="text-lg font-bold mb-4">التواصل مع الإدارة</h2>
            
            <div className="flex-1 overflow-y-auto space-y-4 mb-4 pr-2">
              {details.messages.length === 0 ? (
                <div className="text-center text-surface-500 mt-10">
                  لا توجد رسائل حاليا. يمكنك إرسال استفسار للإدارة أدناه.
                </div>
        ...
        <div className="space-y-6">
          <div className="glass-card p-6">
            <h2 className="text-lg font-bold mb-4">الملخص</h2>
            ...
            {details.submission.extra_data && Object.keys(details.submission.extra_data).length > 0 && (
              <>
                <h2 className="text-lg font-bold mt-6 mb-4">البيانات الإضافية</h2>
