...
export default function ScholarshipEditPage() {
    ...
    const fetchCycle = async () => {
      try {
        ...
        setBasicInfo({
          ...
          application_start: formatForDatetimeLocal(data.application_start),
          application_deadline: formatForDatetimeLocal(data.application_deadline),
          total_quota: data.total_quota || 0
        });

        const config = data.configuration || {};
        setDetails({
          category: config.details?.category || "",
          funding_type: config.details?.funding_type || "",
          academic_level: config.details?.academic_level || "",
          short_description: config.details?.short_description || "",
          about: config.details?.about || "",
        });

        setEligibility({
          min_gpa: config.eligibility?.min_gpa || "",
          nationalities: config.eligibility?.nationalities || "",
          special_conditions: config.eligibility?.special_conditions || ""
        });

        setDocuments(config.documents || []);
        setFormSteps(config.form_steps || []);
        
      } catch (error) {
        console.error("Failed to fetch cycle details", error);
        alert("فشل في تحميل تفاصيل المنحة");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const updateDocument = (docId: string, field: keyof DocumentReq, value: any) => {
    setDocuments(documents.map(d => d.id === docId ? { ...d, [field]: value } : d));
  };
  ...
  const updateStepTitle = (stepId: string, title: string) => {
    setFormSteps(formSteps.map(s => s.id === stepId ? { ...s, title } : s));
  };

  const removeStep = (stepId: string) => {
    setFormSteps(formSteps.filter(s => s.id !== stepId));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await apiClient.put(`/scholarships/cycles/${id}`, payload);
      alert("تم تحديث المنحة بنجاح!");
      router.push("/portal/admin/scholarships");
    } catch (error: any) {
      console.error(error);
      const errMsg = error.response?.data?.error?.message || error.message || "حدث خطأ أثناء التحديث.";
      alert("خطأ: " + errMsg);
    } finally {
      setIsSubmitting(false);
    }
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-surface-500 mt-1">
            تعديل بيانات ونموذج التقديم الخاص بالمنحة الحالية.
          </p>
        </div>
